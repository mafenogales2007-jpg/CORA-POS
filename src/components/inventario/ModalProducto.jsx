...
export default function ModalProducto({ productoAEditar, onCerrar, onGuardadoExitoso }) {
  ...
  useEffect(() => {
    ...
    if (productoAEditar) {
      ...
      setCodigoBarras(productoAEditar.codigo || productoAEditar.codigo_barras || '');
      setCategoria(productoAEditar.categoria_id || productoAEditar.categoria || '');
      setUnidadMedida(productoAEditar.unidad_medida || 'und');
      setStock(productoAEditar.stock || productoAEditar.existencias || 10);
      setCosto(productoAEditar.costo || 0);
      setPrecioVenta(productoAEditar.precio || 0);
      setImagenUrl(productoAEditar.imagen_url || productoAEditar.url_foto || '');
      setAccesoRapido(productoAEditar.acceso_rapido || false);
    }
  }, [productoAEditar]);
  ...
  const handleNombreChange = (e) => {
    ...
    setNombre(val);
    if (val.trim().length >= 2 && !productoAEditar) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (errorSupabase) {
      alert('Error al guardar el producto: ' + errorSupabase.message);
    } else {
    ...
    <div className="modal-overlay">
      <div className="modal-container-ui">
        <div className="modal-header-ui">
          <div>
            <h2 className="title-bold">{productoAEditar ? 'Editar producto' : 'Nuevo producto'}</h2>
          ...
          <div className="col-right-ui">
            ...
            <div className="footer-actions-ui">
              ...
              <button type="submit" className="btn-guardar-ui" disabled={guardando}>
                ...
                {guardando ? 'Guardando...' : (productoAEditar ? 'Actualizar producto' : 'Guardar producto')}
              </button>
            </div>
          </div>
        </form>
